...
const printVersions = function (args, callback) {
  let left, left1;
  const apmVersion =
    (left = require("../package.json").version) != null ? left : "";
  const npmVersion =
    (left1 = require("npm/package.json").version) != null ? left1 : "";
  const nodeVersion =
...
`;
          ...
          if (config.isWin32()) {
            ...
            const visualStudioVersion =
              (left2 = config.getInstalledVisualStudioFlag()) != null
                ? left2
                : "";
...
var getPythonVersion = function (callback) {
  ...
  return npm.load(npmOptions, function () {
    let left;
    let python =
      (left = npm.config.get("python")) != null ? left : process.env.PYTHON;
    if (config.isWin32() && !python) {
      let rootDir =
        process.env.SystemDrive != null ? process.env.SystemDrive : "C:\\";
