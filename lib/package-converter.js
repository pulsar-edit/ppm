...
module.exports = PackageConverter = class PackageConverter {
  ...
  convertSettings(settings) {
    ...
    const editorProperties = _.compactObject({
      commentStart: _.valueForKeyPath(
        settings,
        "shellVariables.TM_COMMENT_START"
      ),
  ...
  convertSnippets(packageName, source) {
    ...
    for (let child of Array.from(fs.readdirSync(sourceSnippets))) {
      var left, selector;
      const snippet =
        (left = this.readFileSync(path.join(sourceSnippets, child))) != null
          ? left
          : {};
      ...
      try {
        ...
        e.message = `In file ${e.fileName} at ${JSON.stringify(scope)}: ${
          e.message
        }`;
        throw e;
      }
