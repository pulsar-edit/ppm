...
module.exports = Install = (function () {
  Install = class Install extends Command {
    ...
    constructor() {
      ...
      this.installGitPackageDependencies =
        this.installGitPackageDependencies.bind(this);
...
`);
      options
        ...
        .describe(
          "compatible",
          "Only install packages/themes compatible with this Atom version"
        );
      options.alias("h", "help").describe("help", "Print this usage message");
      options
        .alias("s", "silent")
        .boolean("silent")
        .describe("silent", "Set the npm log level to silent");
      options
        .alias("q", "quiet")
        .boolean("quiet")
        .describe("quiet", "Set the npm log level to warn");
      options
        .boolean("check")
        .describe("check", "Check that native build tools are installed");
      options
        .boolean("verbose")
        .default("verbose", false)
        .describe("verbose", "Show verbose debug information");
      options
        .string("packages-file")
        .describe(
          "packages-file",
          "A text file containing the packages to install"
        );
      return options
        .boolean("production")
        .describe("production", "Do not install dev dependencies");
    }

    installModule(options, pack, moduleURI, callback) {
      let installDirectory, nodeModulesDirectory;
      const installGlobally =
        options.installGlobally != null ? options.installGlobally : true;
          ...
          if (code === 0) {
            ...
            if (installGlobally) {
              ...
              assert.equal(
                children.length,
                1,
                "Expected there to only be one child in node_modules"
              );
              child = children[0];
...
`;
    ...
    isPackageInstalled(packageName, packageVersion) {
      try {
        let left;
        const { version } =
          (left = CSON.readFileSync(
            CSON.resolve(path.join("node_modules", packageName, "package"))
          )) != null
            ? left
            : {};
    ...
    installRegisteredPackage(metadata, options, callback) {
      ...
      if (!installGlobally) {
        if (
          packageVersion &&
          this.isPackageInstalled(packageName, packageVersion)
        ) {
          callback(null, {});
          return;
        }
      ...
      return this.requestPackage(packageName, (error, pack) => {
        if (error != null) {
          ...
          if (!packageVersion) {
            this.logFailure();
            callback(
              `No available version compatible with the installed Atom version: ${this.installedAtomVersion}`
            );
            return;
          }

          const { tarball } =
            (pack.versions[packageVersion] != null
              ? pack.versions[packageVersion].dist
              : undefined) != null
              ? pack.versions[packageVersion] != null
                ? pack.versions[packageVersion].dist
                : undefined
          ...
          const commands = [];
          commands.push((next) =>
            this.installModule(options, pack, tarball, next)
          );
          if (
            installGlobally &&
            packageName.localeCompare(pack.name, "en", {
              sensitivity: "accent",
            }) !== 0
          ) {
          ...
          commands.push(function ({ installPath }, next) {
            if (installPath != null) {
              metadata = JSON.parse(
                fs.readFileSync(path.join(installPath, "package.json"), "utf8")
              );
    ...
    installLocalPackage(packageName, packagePath, options, callback) {
      if (!options.argv.json) {
        process.stdout.write(
          `Installing ${packageName} from ${packagePath.slice("file:".length)} `
        );
        const commands = [];
        commands.push((next) => {
        ...
        ((name, version) => {
          return commands.push((next) => {
            if (this.repoLocalPackagePathRegex.test(version)) {
              ...
              return this.installRegisteredPackage(
                { name, version },
                options,
                next
              );
    ...
    installDependencies(options, callback) {
      ...
      commands.push((callback) =>
        this.installPackageDependencies(options, callback)
      );
    ...
    getPackageDependencies() {
      try {
        ...
        const { packageDependencies, dependencies } =
          (left = JSON.parse(metadata)) != null ? left : {};
        ...
        for (let packageName in packageDependencies) {
          ...
          const dependencyPath = this.getRepoLocalPackagePath(
            dependencies[packageName]
          );
          const packageDependencyPath =
            this.getRepoLocalPackagePath(packageSpec);
          if (
            !packageDependencyPath ||
            dependencyPath !== packageDependencyPath
          ) {
    ...
    getRepoLocalPackagePath(packageSpec) {
      ...
      const repoLocalPackageMatch = packageSpec.match(
        this.repoLocalPackagePathRegex
      );
    ...
    warmCompileCache(packageName, callback) {
      ...
      return this.getResourcePath((resourcePath) => {
        try {
          const CompileCache = require(path.join(
            resourcePath,
            "src",
            "compile-cache"
          ));
          ...
          const onDirectory = (directoryPath) =>
            path.basename(directoryPath) !== "node_modules";
    ...
    isBundledPackage(packageName, callback) {
      return this.getResourcePath(function (resourcePath) {
        let atomMetadata;
        try {
          atomMetadata = JSON.parse(
            fs.readFileSync(path.join(resourcePath, "package.json"))
          );
        } catch (error) {
        ...
        return callback(
          __guard__(
            atomMetadata != null ? atomMetadata.packageDependencies : undefined,
            (x) => x.hasOwnProperty(packageName)
          )
        );
      });
    }
    ...
    installGitPackage(packageUrl, options, callback) {
      ...
      tasks.push((data, next) => {
        ...
        return this.cloneFirstValidGitUrl(urls, cloneDir, options, (err) =>
          next(err, data)
        );
      });

      tasks.push((data, next) => {
        return this.installGitPackageDependencies(cloneDir, options, (err) =>
          next(err, data)
        );
      });
      ...
      tasks.push(function (data, next) {
        ...
        return CSON.writeFile(data.metadataFilePath, data.metadata, (err) =>
          next(err, data)
        );
      });
    ...
    cloneFirstValidGitUrl(urls, cloneDir, options, callback) {
      return async.detectSeries(
        urls,
        (url, next) => {
          return this.cloneNormalizedUrl(url, cloneDir, options, (error) =>
            next(null, !error)
          );
        },
        function (err, result) {
    ...
    cloneNormalizedUrl(url, cloneDir, options, callback) {
      ...
      return develop.cloneRepository(url, cloneDir, options, (err) =>
        callback(err)
      );
    }
...
`.yellow
      ...
      commands.push((callback) =>
        this.loadInstalledAtomMetadata(() => callback())
      );
      ...
      return async.mapSeries(
        commands,
        iteratee,
        function (err, installedPackagesInfo) {
          if (err) {
            return callback(err);
          }
          installedPackagesInfo = _.compact(installedPackagesInfo);
          installedPackagesInfo = installedPackagesInfo.filter(
            (item, idx) => packageNames[idx] !== "."
          );
          if (options.argv.json) {
