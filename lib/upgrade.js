...
`);
      ...
      options
        .boolean("json")
        .describe("json", "Output outdated packages as a JSON array");
      options
        .string("compatible")
        .describe(
          "compatible",
    ...
    folderIsRepo(pack) {
      const repoGitFolderPath = path.join(
    ...
    getLatestVersion(pack, callback) {
      ...
      return request.get(requestSettings, (error, response, body) => {
        ...
        if (error != null) {
          return callback(
            `Request for package information failed: ${error.message}`
          );
    ...
    upgradePackages(options, callback) {
      ...
      return this.getAvailableUpdates(packages, (error, updates) => {
        ...
        if (options.argv.json) {
          ...
          console.log(
            "Package Updates Available".cyan + ` (${updates.length})`
          );
