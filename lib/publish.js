...
`);
      ...
      options
        ...
        .describe(
          "tag",
          "Specify a tag to publish. Must be of the form vx.y.z"
        );
      return options
        .alias("r", "rename")
        .string("rename")
    ...
    waitForTagToBeAvailable(pack, tag, callback) {
      ...
      const requestSettings = {
        url: `https://api.github.com/repos/${Packages.getRepository(
          pack
        )}/tags`,
        json: true,
      };
    ...
    loadMetadata() {
      ...
      if (!fs.isFileSync(metadataPath)) {
        throw new Error(
          `No package.json file found at ${process.cwd()}/package.json`
        );
      }
    ...
    loadRepository() {
      ...
      if (
        !(repo != null ? repo.isWorkingDirectory(currentDirectory) : undefined)
      ) {
        throw new Error(
            ...
            return this.spawn(
              ...
              (code, stderr, stdout) => {
                ...
                if (code !== 0) {
                  ...
                  return callback(
                    `\`git add package.json\` failed: ${addOutput}`
                  );
                }
                ...
                return this.spawn(
                  ...
                  (code, stderr, stdout) => {
                    ...
                    if (code === 0) {
                      ...
                      return callback(
                        `Failed to commit package.json: ${commitOutput}`
                      );
                    }
                  }
                );
