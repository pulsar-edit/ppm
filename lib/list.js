...
module.exports = List = (function () {
  List = class List extends Command {
    ...
    constructor() {
      let configPath;
      super();
      this.userPackagesDirectory = path.join(
      ...
      if (
        (configPath = CSON.resolve(
          path.join(config.getAtomDirectory(), "config")
        ))
      ) {
        try {
...
`);
      ...
      options
        .boolean("disabled")
        .describe("disabled", "Print only disabled packages");
    ...
    logPackages(packages, options) {
      if (options.argv.bare) {
        ...
        tree(packages, (pack) => {
          ...
          if (
            (pack.apmInstallSource != null
              ? pack.apmInstallSource.type
              : undefined) === "git"
          ) {
      ...
      for (let child of Array.from(fs.list(directoryPath))) {
        ...
        if (
          (manifestPath = CSON.resolve(
            path.join(directoryPath, child, "package")
          ))
        ) {
          try {
    ...
    listUserPackages(options, callback) {
      const userPackages = this.listPackages(
    ...
    listGitPackages(options, callback) {
      const gitPackages = this.listPackages(
        this.userPackagesDirectory,
        options
      ).filter(
        (pack) =>
      ...
      return config.getResourcePath((resourcePath) => {
        ...
        if (!options.argv.bare && !options.argv.json) {
          if (options.argv.themes) {
            ...
            console.log(
              `${"Built-in Atom Packages".cyan} (${packages.length})`
            );
          }
        }
