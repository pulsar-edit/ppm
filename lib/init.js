...
`);
      ...
      options
        ...
        .describe(
          "syntax",
          "Sets package syntax to CoffeeScript or JavaScript"
        );
      options
        .alias("t", "theme")
        .string("theme")
    ...
    run(options) {
      ...
      if (
        (options.argv.package != null
          ? options.argv.package.length
          : undefined) > 0
      ) {
        ...
        (options.argv.language != null
          ? options.argv.language.length
          : undefined) > 0
      ) {
    ...
    generateFromTemplate(packagePath, templatePath, packageName) {
      ...
      return (() => {
        ...
        for (let childPath of Array.from(fs.listRecursive(templatePath))) {
          ...
          relativePath = this.replacePackageNamePlaceholders(
            relativePath,
            packageName
          );
          ...
          if (fs.isDirectorySync(templateChildPath)) {
            ...
            contents = this.replacePackageNamePlaceholders(
              contents,
              packageName
            );
            contents = this.replacePackageAuthorPlaceholders(
              contents,
              packageAuthor
            );
    ...
    replacePackageNamePlaceholders(string, packageName) {
      const placeholderRegex =
        /__(?:(package-name)|([pP]ackageName)|(package_name))__/g;
