...
`);
      ...
      return options
        .boolean("json")
        .describe("json", "Output packages as a JSON array");
    }
      ...
      return request.get(requestSettings, function (error, response, body) {
        ...
        if (error != null) {
          ...
          let packages = body.filter(
            (pack) =>
              __guard__(
                pack != null ? pack.releases : undefined,
                (x) => x.latest
              ) != null
          );
          packages = packages.map(
            ({ readme, metadata, downloads, stargazers_count }) =>
    ...
    run(options) {
      ...
      const user =
        options.argv.user != null
          ? options.argv.user.toString().trim()
          : undefined;

      return this.getStarredPackages(
        user,
