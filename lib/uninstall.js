...
`);
      ...
      return options
        ...
        .describe(
          "hard",
          "Uninstall from ~/.atom/packages and ~/.atom/dev/packages"
        );
    }
    ...
    run(options) {
      ...
      const devPackagesDirectory = path.join(
        config.getAtomDirectory(),
        "dev",
        "packages"
      );
        ...
        try {
          ...
          if (!options.argv.dev) {
            ...
            if (fs.existsSync(packageManifestPath)) {
              ...
              throw new Error(
                `No package.json found at ${packageManifestPath}`
              );
            }
          }
          ...
          this.logFailure();
          uninstallError = new Error(
            `Failed to delete ${packageName}: ${error.message}`
          );
          break;
        }
      }
