...
describe("apm upgrade", function() {
  ...
  beforeEach(function() {
    ...
    const app = express();
    app.get('/packages/test-module', (request, response) => response.sendFile(path.join(__dirname, 'fixtures', 'upgrade-test-module.json')));
    app.get('/packages/multi-module', (request, response) => response.sendFile(path.join(__dirname, 'fixtures', 'upgrade-multi-version.json')));
    app.get('/packages/different-repo', (request, response) => response.sendFile(path.join(__dirname, 'fixtures', 'upgrade-different-repo.json')));
  ...
  describe("for outdated git packages", function() {
    ...
    it('shows an upgrade plan', () => apmRun(['upgrade', '--list', '--no-color'], function() {
    ...
    return it('updates to the latest sha', () => apmRun(['upgrade', '-c', 'false', 'test-git-repo'], function() {
      const json = JSON.parse(fs.readFileSync(pkgJsonPath), 'utf8');
      return expect(json.apmInstallSource.sha).toBe('8ae432341ac6708aff9bb619eb015da14e9d0c0f');
    }));
  });

  return describe("for outdated git packages (when HEAD points to main)", function() {
    let [pkgJsonPath] = Array.from([]);
    ...
    return it('updates to the latest sha', () => apmRun(['upgrade', '-c', 'false', 'test-git-repo-with-main'], function() {
      const json = JSON.parse(fs.readFileSync(pkgJsonPath), 'utf8');
      return expect(json.apmInstallSource.sha).toBe('c81278af71de6c12ce0bc02936d5c1eb22aadaf9');
    }));
  });
});
