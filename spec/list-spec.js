...
const createFakePackage = function(type, metadata) {
  const packagesFolder = (() => { switch (type) {
    case "user": case "git": return "packages";
    case "dev": return path.join("dev", "packages");
  } })();
...
describe('apm list', function() {
  ...
  it('lists the installed packages', () => listPackages([], function() {
  ...
  it('lists the installed packages without versions with --no-versions', () => listPackages(['--no-versions'], function() {
    const lines = console.log.argsForCall.map(arr => arr.join(' '));
    expect(lines[0]).toMatch(/Built-in Atom Packages.*1/);
  ...
  describe('enabling and disabling packages', function() {
    ...
    it('labels disabled packages', () => listPackages([], () => expect(console.log.argsForCall[1][0]).toContain('test-module@1.0.0 (disabled)')));

    it('displays only disabled packages when --disabled is called', () => listPackages(['--disabled'], function() {
    ...
    return it('displays only enabled packages when --enabled is called', () => listPackages(['--enabled'], function() {
  ...
  it('lists packages in json format when --json is passed', () => listPackages(['--json'], function() {
  ...
  it('lists packages in bare format when --bare is passed', () => listPackages(['--bare'], function() {
    const lines = console.log.argsForCall.map(arr => arr.join(' '));
  ...
  it('list packages in bare format without versions when --bare --no-versions is passed', () => listPackages(['--bare', '--no-versions'], function() {
    const lines = console.log.argsForCall.map(arr => arr.join(' '));
