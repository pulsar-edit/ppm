...
describe("apm init", function() {
  ...
  describe("when creating a package", function() {
    describe("when package syntax is CoffeeScript", () => it("generates the proper file structure", function() {
      ...
      return runs(function() {
        ...
        return expect(JSON.parse(fs.readFileSync(path.join(packagePath, 'package.json'))).repository).toBe('https://github.com/somebody/fake-package');
      });
    }));

    describe("when package syntax is JavaScript", () => it("generates the proper file structure", function() {
      const callback = jasmine.createSpy('callback');
    ...
    describe("when package syntax is unsupported", () => it("logs an error", function() {
      const callback = jasmine.createSpy('callback');
    ...
    return describe("when converting a TextMate bundle", function() {
      ...
      it("generates the proper file structure", function() {
        ...
        return expect(CSON.readFileSync(path.join(packagePath, 'settings', 'fake-package.cson'))['.source.r']['editor']).toEqual({
          foldEndPattern: '(^\\s*\\)|^\\s*\\})',
          commentStart: '# '
        });
    });
      ...
      return it("unescapes escaped dollar signs `$` in snippets", function() {
        let forLoopBody = CSON.readFileSync(path.join(packagePath, 'snippets', 'fake-package.cson'))['.source.perl']['For Loop'].body;
  ...
  describe("when creating a theme", function() {
    it("generates the proper file structure", function() {
      ...
      return runs(function() {
        ...
        return expect(JSON.parse(fs.readFileSync(path.join(themePath, 'package.json'))).repository).toBe('https://github.com/somebody/fake-theme');
      });
    });
  ...
  return describe("when creating a language", function() {
    it("generates the proper file structure", function() {
      ...
      return runs(function() {
        ...
        return expect(JSON.parse(fs.readFileSync(path.join(languagePath, 'package.json'))).repository).toBe('https://github.com/somebody/language-fake');
      });
    });
